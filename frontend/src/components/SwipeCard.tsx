...
export default function SwipeCard({ card }: Props) {
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-auto select-none cursor-grab active:cursor-grabbing">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h3 className="font-semibold text-gray-800 text-lg">{card.displayName}</h3>
          <p className="text-sm text-gray-500">{card.role} {card.department ? `— ${card.department}` : ''}</p>
        </div>
      </div>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-lg font-bold text-primary-600">{Math.round(card.matchScore * 100)}%</span>
        </div>
      ...
      {card.sharedSkills.length > 0 && (
        <div className="mb-3">
          <p className="text-xs text-gray-500 mb-1">💼 Competenze in comune</p>
      ...
      {card.sharedAiTools.length > 0 && (
        <div className="mb-3">
          <p className="text-xs text-gray-500 mb-1">🤖 AI Tools in comune</p>
          <div className="flex flex-wrap gap-1">
            {card.sharedAiTools.slice(0, 4).map((t) => (
              <span key={t} className="px-2 py-0.5 bg-purple-50 text-purple-700 rounded-full text-xs">{t}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {card.sharedInterests.length > 0 && (
        <div>
          <p className="text-xs text-gray-500 mb-1">❤️ Interessi in comune</p>
          <div className="flex flex-wrap gap-1">
            {card.sharedInterests.slice(0, 4).map((i) => (
              <span key={i} className="px-2 py-0.5 bg-pink-50 text-pink-700 rounded-full text-xs">{i}</span>
            ))}
          </div>
        </div>
      )}
